...
Our greedy multi-path routing algorithm allows multi-user routing with congestion mitigation via quantum memories, with algorithmic efficiency ~$O(M^3 V^2)$~, for M user-pairs on a V-vertex graph, and is therefore highly scalable and efficient in both users and network size.
